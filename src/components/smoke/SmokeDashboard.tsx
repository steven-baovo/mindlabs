...
import { initSmokeSessionWithSurvey, incrementCravingDefeated, resetSmokeSession } from '@/app/(frontend)/smoke/actions'
import { Shield, Brain, Activity, AlertTriangle, AlertCircle } from 'lucide-react'
...
const MILESTONES = [
  {
    day: 3,
    label: 'Ngày 3',
    title: 'Thanh lọc vật lý hoàn tất',
    description: 'Nicotine đã được đào thải hoàn toàn khỏi máu. Cơ thể bạn chính thức không còn lệ thuộc về mặt thể chất.',
  },
  {
    day: 7,
    label: 'Ngày 7',
  ...
  },
  {
    ...
    description: 'Đỉnh điểm của sự bứt rứt tâm lý đã qua. Não bộ đang học lại cách tự sản sinh Dopamine mà không cần chất kích thích.',
  },
  {
    day: 21,
    label: 'Ngày 21',
    title: 'Tái thiết lập thần kinh',
    description: 'Đường mòn thần kinh cũ đã suy yếu đáng kể. Bạn đã xây dựng được một thói quen mới: sống không cần khói thuốc.',
  },
]

// ─── INITIATION RITUAL ──────────────────────────────────────────────────────
function InitiationRitual({ onComplete }: { onComplete: (data: { cigarettesPerDay: number; yearsSmoked: number }) => Promise<void> }) {
  ...
  const canProceedStep1 = cigarettes !== '' && years !== '' && parseInt(cigarettes) > 0 && parseInt(years) > 0
        ...
        {step === 1 && (
          <div className="space-y-6">
            <div>
              ...
              <p className="text-gray-500 text-sm">Hai con số này sẽ giúp hệ thống hiệu chỉnh lộ trình phù hợp với bạn, không phải cho đại đa số.</p>
            </div>
            <div className="space-y-4">
              <div>
                ...
                />
                <p className="text-xs text-gray-400 mt-1">Để hệ thống đánh giá mức độ bám rễ của thói quen trong não bộ.</p>
              </div>
              <div>
        ...
        {step === 2 && (
          <div className="space-y-6">
            <div>
              ...
              <p className="text-gray-500 text-sm">Đây không phải cam kết vô thời hạn. Chỉ 21 ngày. Dưới đây là những gì sẽ xảy ra với não bộ và cơ thể bạn.</p>
            </div>
            <div className="space-y-4">
              {MILESTONES.map((m) => (
                <div key={m.day} className="flex gap-4 items-start">
                  <div className="shrink-0 w-12 h-12 rounded-full bg-gray-100 flex items-center justify-center font-mono font-bold text-[#1a2b49] text-sm">{m.day}</div>
                  <div>
          ...
          <div className="space-y-6">
            <div>
              ...
              <p className="text-gray-500 text-sm">
                Bạn đã hút <span className="font-mono font-bold text-[#1a2b49]">{cigarettes} điếu/ngày</span> trong <span className="font-mono font-bold text-[#1a2b49]">{years} năm</span>. Bộ máy thần kinh đã được lập trình sâu. 21 ngày tiếp theo sẽ không dễ dàng. Nếu bạn thực sự sẵn sàng, hãy gõ câu cam kết bên dưới để khởi động hệ thống.
              </p>
            </div>
            <div>
            ...
            <div className="flex gap-3">
              ...
              <button
                ...
                disabled={confirmText !== COMMIT_PHRASE || isSubmitting}
...
export default function SmokeDashboard({ initialState }: { initialState: SmokeState }) {
  ...
  useEffect(() => {
    ...
    if (isSOSActive && sosTimeLeft > 0) {
      interval = setInterval(() => setSosTimeLeft((p) => p - 1), 1000)
    } else if (isSOSActive && sosTimeLeft === 0) {
      handleSOSSuccess()
    }
    return () => clearInterval(interval)
  ...
  if (!state) {
    return <InitiationRitual onComplete={handleInit} />
  }
    ...
    <div className="space-y-6">
      ...
      {isExtinctionBurst && (
        <div className="bg-red-50 border-l-4 border-red-500 p-4 rounded-r-xl flex gap-4 items-start">
          ...
          <div>
            ...
            <p className="text-red-700 text-xs mt-1 leading-relaxed">Trong 72 giờ tới, não bộ của bạn sẽ kích hoạt những cơn thèm mạnh nhất, phi lý nhất — đây là nỗ lực cuối cùng của nó trước khi buông bỏ hoàn toàn. Bạn đã được báo trước. Hãy đứng vững.</p>
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-8 shadow-sm flex flex-col items-center">
          ...
          <div className="font-mono text-4xl md:text-5xl font-bold text-[#1a2b49] tracking-tight tabular-nums flex gap-2 items-end">
            <div className="flex flex-col items-center gap-1">
              <span>{timeElapsed.days.toString().padStart(2, '0')}</span>
              <span className="text-xs font-sans text-gray-400 font-normal uppercase tracking-widest">Ngày</span>
            </div>
            <span className="text-gray-200 mb-5">:</span>
            <div className="flex flex-col items-center gap-1">
              <span>{timeElapsed.hours.toString().padStart(2, '0')}</span>
              <span className="text-xs font-sans text-gray-400 font-normal uppercase tracking-widest">Giờ</span>
            </div>
            <span className="text-gray-200 mb-5">:</span>
            <div className="flex flex-col items-center gap-1">
              <span>{timeElapsed.minutes.toString().padStart(2, '0')}</span>
              <span className="text-xs font-sans text-gray-400 font-normal uppercase tracking-widest">Phút</span>
            </div>
            <span className="text-gray-200 mb-5">:</span>
            <div className="flex flex-col items-center gap-1">
              <span>{timeElapsed.seconds.toString().padStart(2, '0')}</span>
              <span className="text-xs font-sans text-gray-400 font-normal uppercase tracking-widest">Giây</span>
            </div>
          </div>
        ...
        <div className="bg-[#1a2b49] border border-blue-900 rounded-xl p-8 shadow-sm flex flex-col items-center text-white">
          ...
          <p className="text-xs text-blue-200 mt-4 text-center max-w-xs leading-relaxed">
            Chỉ số này không tự tăng. Khi cơn thèm ập đến, nhấn nút SOS bên dưới và đứng vững qua 5 phút. Mỗi lần bạn thắng, con số này tăng thêm 1.
          </p>
        </div>
      </div>

      {/* Row 2: Timeline 21 Days */}
      <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm">
        ...
        <div className="relative">
          ...
          <div
            className="absolute top-5 left-5 h-0.5 bg-[#1a2b49] z-0 transition-all duration-1000"
            style={{ width: `${Math.min(100, (daysElapsed / 21) * 100)}%` }}
          />
          <div className="relative z-10 flex justify-between">
            {MILESTONES.map((m) => {
              const reached = daysElapsed >= m.day
              return (
                <div key={m.day} className="flex flex-col items-center gap-2 max-w-[80px]">
                  ...
                  <div className={`text-xs text-center leading-relaxed hidden md:block transition-colors ${reached ? 'text-gray-600' : 'text-gray-300'}`}>{m.title}</div>
                </div>
              )
            })}
          </div>
      ...
      <div className="pt-2">
        <button
          onClick={() => { setIsSOSActive(true); setSosTimeLeft(300) }}
          className="w-full py-5 border-2 border-[#1a2b49] bg-white hover:bg-[#1a2b49] text-[#1a2b49] hover:text-white rounded-xl font-bold text-lg uppercase tracking-widest transition-all flex items-center justify-center gap-3 group"
        >
        ...
        <div className="fixed inset-0 z-50 bg-black/80 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl max-w-md w-full p-6 shadow-2xl">
            ...
            <div className="flex gap-3">
              ...
              <button
                onClick={handleReset}
                disabled={resetInput !== 'TÔI CHẤP NHẬN BẮT ĐẦU LẠI' || isSubmitting}
